Here is the upgraded `README.md` reflecting the latest file structure and tooling additions based on the provided repository contents:
...
- Stack: React (frontend, now powered by Vite) + FastAPI (backend) + PostgreSQL + Groq-hosted open-source LLM (Llama 3.1 / Qwen2.5)
...
- ✅ React chat UI (talks to `/chat`), initialized with Vite and utilizing Oxlint for linting
- ✅ Custom visual assets integration (`hero.png`, `icons.svg`)
- ✅ Docker Compose for local dev (Postgres + backend), including a dedicated `Dockerfile` for the backend
...
- Phase 5: paper trading deployment gate, guardrails against bad AI-generated code, full Docker packaging for the frontend stack
...
*(Note: Code quality is enforced using Oxlint; configurations are set in `.oxlintrc.json`)*
...
*(The backend `Dockerfile` is included and automatically managed by the compose file).*
